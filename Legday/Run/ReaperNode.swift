...
/// U23 presentation over U19's duel state: the Reaper and its telegraphs.
final class ReaperNode: SKNode {
    ...
    init(texture: SKTexture) {
        super.init()
        // The duel keeps a low camera and the drawing carries its own ink, so
        // this one is never tinted.
        body.texture = texture
        body.size = texture.size()
        body.position = .zero
        addChild(body)

        telegraphLine.strokeColor = SKColor(white: 0.9, alpha: 0.5)
        telegraphLine.lineWidth = 2
        telegraphRing.strokeColor = SpriteAtlas.rgb(0xC99A2E)
    ...
    func update(duel: DuelState, fogTopY: CGFloat, sceneSize: CGSize) {
        ...
        body.color = decay > 0.6 ? SpriteAtlas.rgb(0x241C12)
            : SpriteAtlas.rgb(0x3A2A1E)
    ...
    private func color(for pattern: ReaperPattern) -> SKColor {
        ...
        case .slam: return SpriteAtlas.rgb(0xC99A2E)
        case .fogSurge: return SpriteAtlas.rgb(0x8A6FB3)
        }
    }
}
